import { FaEye, FaDownload, FaUser, FaIdCard, FaCar } from "react-icons/fa";

export default function DocumentsTab({ vehicleOwner }: { vehicleOwner: any }) {
    ...
    const uploadedDocumentUrl = profile?.uploadedDocumentUrl || profile?.driverPassport;

    // Create document items from actual data
    const documentItems = [
        {id:1, title:"Profile Picture", img:profileImage, fallbackText:"No Profile Picture", icon:<FaUser className="text-primary" />},
        {id:2, title:identificationType || "Identification Document", img:uploadedDocumentUrl, fallbackText:"No Document Uploaded", icon:<FaIdCard className="text-success" />},
        {id:3, title:"Vehicle Document", img:"", fallbackText:"No Vehicle Document", icon:<FaCar className="text-warning" />} // Placeholder for vehicle docs
    ];
                            ...
                            <div className="d-flex justify-content-between align-items-center mb-2">
                                ...
                                <div className="d-flex gap-2">
                                    <button 
                                        ...
                                        onClick={() => handleViewDocument(doc.img, doc.title)} 
                                        disabled={!doc.img}
                                    >
                                        <FaEye />
                                    </button>
                                    <button 
                                        ...
                                        onClick={() => handleDownloadDocument(doc.img, doc.title)} 
                                        disabled={!doc.img}
                                    >
                                        <FaDownload />
                                    </button>
