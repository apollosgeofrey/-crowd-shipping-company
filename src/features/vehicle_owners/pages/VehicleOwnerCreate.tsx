...
export default function VehicleOwnerCreate() {
    // form states
    ...
    const handleClear = () => setFormData({fullName:"", email: "", phoneNumber: "", password: "", confirmPassword: "", status: "active"});

    // form submit handler for creating vehicle owner
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const res = await vehicleOwnerApi.createVehicleOwner(formData);
            if (res.code === 201) {
                setMessage({ type: "success", text: res.message || "Vehicle Owner created successfully." });
                Swal.fire("Success", (res.message || "Vehicle Owner created successfully."), "success");
                handleClear();
            } else {
                setMessage({ type: "error", text: res.message || "Failed to create vehicle owner." });
                Swal.fire("Error", (res.message || "Failed to create vehicle owner."), "error");
            }
        } catch (err: any) {
                            ...
                            <div className="card shadow-sm border-0 p-2">
                                ...
                                <div className="d-flex mb-4">
                                    <button type="button" className="btn btn-link text-decoration-none text-primary fw-semibold border-bottom" style={{ borderBottom: "3px solid #E35D3F" }}>
                                        <h5><i className="fa fa-car me-1"></i> Basic Information</h5>
                                    </button>
                                        ...
                                        <div className="d-flex justify-content-end gap-2 mt-4 pt-3 border-top">
                                            ...
                                            <button type="submit" disabled={loading} className="btn btn-sm btn-primary" style={{ backgroundColor: "#E35D3F", borderColor: "#E35D3F" }}>
                                                {loading ? (
                                                    ...
                                                    <><span className="fa fa-save"></span> Create Vehicle Owner</>
                                                )}
                                            </button>
                                        </div>
                                    </form>
